...
function load(){ try{ const j=JSON.parse(localStorage.getItem(SAVE_KEY)); if(j){ Object.assign(S,j); S.lv=Object.assign({},LV0, j.lv||{}); S.towers=Array.isArray(j.towers)&&j.towers.length>=9? j.towers : defaultTowers(); for(const t of S.towers){ if(t.fixed&&Math.abs(t.a)===4.8) t.a=Math.sign(t.a)*5.2; } S.paid=j.paid||{}; } }catch(e){} }
...
const pileStones=[]; function setStonePile(n){ n=Math.min(n,18); while(pileStones.length<n){ const i=pileStones.length; const row=Math.floor(i/6), col=i%6; const b=mesh(G.box,M.rock,0.42,0.3,0.42); b.position.set(-1.9+col*0.46+(row%2)*0.2,0.45+row*0.3,0.55); b.rotation.y=rand(-.3,.3); depot.add(b); pileStones.push(b);} while(pileStones.length>n){ depot.remove(pileStones.pop()); } }
...
let bubbleKey='';
function updateBubble(){ const pd=bubblePad; if(!pd){ bubbleEl.style.display='none'; bubbleKey=''; return; } if(pd.locked){ const key='L'+pd.def.id; if(key!==bubbleKey){ bubbleKey=key; bubbleEl.innerHTML=`<b>🔒 ${pd.def.name}</b><span>${pd.def.desc}</span><em>${pd.def.lock} ile açılır</em>`; } bubbleEl.style.display='block'; const p=player.g.position; v3.set(p.x,4.6,p.z).project(camera); bubbleEl.style.left=((v3.x+1)/2*innerWidth)+'px'; bubbleEl.style.top=((1-v3.y)/2*innerHeight)+'px'; return; } const lvl=padLevel(pd.def); const cost=padCost(pd); const cur=S.paid[pd.def.id]||0; const need=Math.max(0,Math.ceil(cost-cur)); const pr=padRes(pd); const wood=pr==='wood'; const res=pr==='stone'?'taş':wood?'odun':'altın'; const have=pr==='stone'?S.stones+S.stone:wood?S.logs+S.wood:S.coins; const can=have>=need-0.01;
...
const fliers=[];
function fly(from,toObj,onDone,logLike,speed){ const m=logLike==='stone'?mesh(G.box,M.rock,0.42,0.32,0.42,false):logLike?mesh(G.log,M.log,1,1,1,false):mesh(G.cyl,M.coin,0.3,0.1,0.3,false); if(logLike===true) m.rotation.z=Math.PI/2; else if(!logLike) m.rotation.x=Math.PI/2; m.position.copy(from); scene.add(m); fliers.push({m,toObj,t:0,from:from.clone(),onDone,spin:rand(4,9),speed:speed||3.4,logLike}); }
...
function storeStone(fromPos){ fly(fromPos.clone().setY(1),DEPOT.clone().add(new THREE.Vector3(0,0.8,0.6)),()=>{ S.stone++; setStonePile(Math.min(18,S.stone)); SFX.sell(); },'stone',4); }
...
const PADS=[
  ...
  {id:'wall', lock:'Kuzey kulesi kur', name:'Sur', desc:'Sur ve kapılar güçlenir; 4. seviyeden sonra taş ister', res:l=>l>=3?'stone':'wood', pos:P(-3.2,-3.2), kind:'wall', key:'wall', cost:l=>l>=3?Math.round(30*Math.pow(1.45,l-3)):Math.round(40*Math.pow(1.55,l)), max:MAXL, show:()=>S.towers[1].lvl>=1},
  {id:'gate', lock:'Sur 1', name:'Kapı Onarım', desc:'Kapıların canı artar; ileri seviyeler taş ister', res:l=>l>=3?'stone':'wood', pos:P(-5.9,-3.2), kind:'up', key:'gateLv', cost:l=>l>=3?Math.round(28*Math.pow(1.4,l-3)):Math.round(50*Math.pow(1.4,l)), max:MAXL, show:()=>S.lv.wall>=1},
  {id:'expand', lock:'İki kuzey kulesi', name:'Genişlet', desc:'Sur kare olarak büyür; 4. kademeden sonra taş ister', res:l=>l>=3?'stone':'wood', pos:P(-8.6,-3.2), kind:'expand', key:'expand', cost:l=>l>=3?Math.round(45*Math.pow(1.5,l-3)):Math.round(60*Math.pow(1.6,l)), max:MAXL, show:()=>S.towers[1].lvl>=1&&S.towers[2].lvl>=1},
  {id:'newTower', lock:'Genişlet 1', name:'Yeni Okçu Kulesi', desc:'Sur kenarında istediğin yere kurarsın', res:'gold', pos:P(-3.2,-5.9), kind:'newTower', tk:'a', cost:l=>Math.round(120*Math.pow(1.35,l)), max:MAXL, show:()=>S.lv.expand>=1, lvl:()=>nFree('a')},
  {id:'newCannon', lock:'Genişlet 1', name:'Yeni Topçu Kulesi', desc:'Gülle atar, toplu hasar verir; yerini sen seçersin', res:'gold', pos:P(-5.9,-5.9), kind:'newTower', tk:'c', cost:l=>Math.round(160*Math.pow(1.4,l)), max:MAXL, show:()=>S.lv.expand>=1, lvl:()=>nFree('c')},
  {id:'cannonTrain', lock:'Bir topçu kulesi', name:'Topçu Talimi', desc:'Tüm topçu kulelerinin hasarı artar', res:'gold', pos:P(-8.6,-5.9), kind:'up', key:'cannonTrain', cost:l=>Math.round(100*Math.pow(1.3,l)), max:MAXL, show:()=>nFree('c')>=1},
  // Kuzeydoğu: savunma
  ...
  {id:'soldierTrain', lock:'Bir asker', name:'Asker Talimi', desc:'Askerler daha sert ve hızlı vurur', res:'gold', pos:P(5.9,-3.2), kind:'up', key:'soldierTrain', cost:l=>Math.round(50*Math.pow(1.3,l)), max:MAXL, show:()=>S.lv.soldier>=1},
  {id:'sword', lock:'Bir asker', name:'Kılıç', desc:'Kılıç hasarı ve menzili artar', res:'gold', pos:P(8.6,-3.2), kind:'up', key:'sword', cost:l=>Math.round(40*Math.pow(1.3,l)), max:MAXL, show:()=>S.lv.soldier>=1},
  {id:'towerTrain', lock:'Kuzey kulesi kur', name:'Okçu Talimi', desc:'Tüm okçu kulelerinin hasarı artar', res:'gold', pos:P(3.2,-5.9), kind:'up', key:'towerTrain', cost:l=>Math.round(80*Math.pow(1.3,l)), max:MAXL, show:()=>S.towers[1].lvl>=1},
  {id:'range', lock:'Genişlet 1', name:'Kule Menzili', desc:'Kuleler daha uzağa atar', res:'gold', pos:P(5.9,-5.9), kind:'up', key:'range', cost:l=>Math.round(120*Math.pow(1.3,l)), max:MAXL, show:()=>S.lv.expand>=1},
  {id:'collector', lock:'Bir asker', name:'Toplayıcı', desc:'Yerdeki miğferleri toplayıp tezgâha taşır', res:'gold', pos:P(8.6,-5.9), kind:'collector', key:'collector', cost:l=>Math.round(70*Math.pow(1.5,l)), max:MAXL, show:()=>S.lv.soldier>=1},
  ...
  {id:'worker', lock:'Balta ya da sırt', name:'Oduncu', desc:'Senin yerine ağaç kesip depoya taşır', res:'gold', pos:P(-8.6,3.2), kind:'worker', key:'worker', cost:l=>Math.round(60*Math.pow(1.5,l)), max:MAXL, show:()=>S.lv.axe+S.lv.bag>=1},
  {id:'feet', lock:'Bir oduncu', name:'Midilli', desc:'Midilli daha hızlı koşar', res:'gold', pos:P(-3.2,5.9), kind:'up', key:'feet', cost:l=>Math.round(40*Math.pow(1.3,l)), max:MAXL, show:()=>S.lv.worker>=1},
  {id:'magnet', lock:'Bir oduncu', name:'Mıknatıs', desc:'Altın ve ganimeti daha uzaktan çeker', res:'gold', pos:P(-5.9,5.9), kind:'up', key:'magnet', cost:l=>Math.round(60*Math.pow(1.3,l)), max:MAXL, show:()=>S.lv.worker>=1},
  {id:'stoneWorker', lock:'Sur 2 ya da genişlet 2', name:'Taşçı', desc:'Taş ocağından taş çıkarıp depoya taşır', res:'gold', pos:P(-3.2,8.6), kind:'stoneWorker', key:'stoneWorker', cost:l=>Math.round(80*Math.pow(1.5,l)), max:MAXL, show:()=>S.lv.wall>=2||S.lv.expand>=2},
  {id:'worker2', lock:'İki oduncu', name:'Oduncu Hızı', desc:'Oduncular daha hızlı yürür', res:'gold', pos:P(-8.6,5.9), kind:'up', key:'workerSpd', cost:l=>Math.round(90*Math.pow(1.3,l)), max:MAXL, show:()=>S.lv.worker>=2},
  // Güneydoğu: ticaret
  {id:'depot', lock:'Bir oduncu', name:'Odun Deposu', desc:'Oduncular daha çok taşır', res:'gold', pos:P(3.2,4.4), kind:'up', key:'depotLv', cost:l=>Math.round(80*Math.pow(1.4,l)), max:MAXL, show:()=>S.lv.worker>=1},
  {id:'trader', lock:'Bir asker', name:'Tüccar', desc:'Tezgâhta satış daha hızlı, fiyat daha yüksek', res:'gold', pos:P(3.2,7.4), kind:'up', key:'trader', cost:l=>Math.round(80*Math.pow(1.3,l)), max:MAXL, show:()=>S.lv.soldier>=1},
  {id:'price', lock:'Tüccar 1', name:'Ganimet Fiyatı', desc:'Müşteriler miğfere daha çok öder', res:'gold', pos:P(5.9,7.4), kind:'up', key:'price', cost:l=>Math.round(120*Math.pow(1.3,l)), max:MAXL, show:()=>S.lv.trader>=1},
];
...
function drawLocked(pd,name,hint){ const x=pd.tex.c.getContext('2d'); x.clearRect(0,0,256,256); x.fillStyle='rgba(40,44,50,0.55)'; rr(x,10,10,236,236,22); x.fill(); x.strokeStyle='rgba(255,255,255,0.45)'; x.lineWidth=6; x.setLineDash([22,14]); rr(x,14,14,228,228,20); x.stroke(); x.setLineDash([]);
  x.fillStyle='rgba(255,255,255,0.85)'; x.textAlign='center'; let fs=26; x.font=`bold ${fs}px "Baloo 2","Nunito",sans-serif`; while(x.measureText(name).width>210&&fs>16){ fs-=2; x.font=`bold ${fs}px "Baloo 2","Nunito",sans-serif`; } x.fillText(name,128,62);
  x.fillStyle='#d9d4c8'; rr(x,104,92,48,40,8); x.fill(); x.strokeStyle='#d9d4c8'; x.lineWidth=8; x.beginPath(); x.arc(128,92,18,Math.PI,0); x.stroke(); x.fillStyle='#3a3f48'; x.beginPath(); x.arc(128,110,6,0,7); x.fill();
  x.fillStyle='#ffd75e'; fs=22; x.font=`800 ${fs}px "Baloo 2","Nunito",sans-serif`; while(x.measureText(hint).width>216&&fs>13){ fs-=1; x.font=`800 ${fs}px "Baloo 2","Nunito",sans-serif`; } x.fillText(hint,128,178); x.fillStyle='rgba(255,255,255,0.7)'; x.font='700 17px "Nunito",sans-serif'; x.fillText('ile açılır',128,204); pd.tex.tex.needsUpdate=true; }
function drawPad(pd,name,lvlTxt,cur,cost,can){ const x=pd.tex.c.getContext('2d'); x.clearRect(0,0,256,256); const wood=padRes(pd)==='wood';
  x.fillStyle=can?(padRes(pd)==='stone'?'rgba(90,92,100,0.92)':wood?'rgba(120,80,30,0.92)':'rgba(30,110,60,0.9)'):'rgba(35,50,40,0.75)'; rr(x,10,10,236,236,22); x.fill();
  ...
  if(padRes(pd)==='stone'){ x.fillStyle='#b9b3a6'; rr(x,104,194,48,30,8); x.fill(); x.strokeStyle='#6f6a60'; x.lineWidth=3; x.stroke(); x.fillStyle='#d6d0c4'; rr(x,112,198,20,10,3); x.fill(); }
  else if(wood){ x.fillStyle='#c98d4e'; rr(x,98,196,60,26,12); x.fill(); x.fillStyle='#f0cd9c'; x.beginPath(); x.arc(158,209,13,0,7); x.fill(); x.strokeStyle='#8a5a30'; x.lineWidth=3; x.stroke(); }
...
function updateWorkers(dt){
  for(const w of workers){ const g=w.guy.g, p=g.position;
    ...
    else if(w.state==='toDepot'){ if(walkTo(w,DEPOT.x,DEPOT.z+2.2,2.2,dt)){ if(w.logs>0){ w.sellT=(w.sellT||0)-dt; if(w.sellT<=0){ w.sellT=0.08; w.logs--; if(st){ setStones(w.guy,w.logs); storeStone(p); } else { setLogs(w.guy,w.logs); storeLog(p); } } } else w.state='idle'; } }
...
function updateEnemies(dt){
  ...
  else if(!waveActive){ if(S.level===1&&S.wave===1&&S.towers[1].lvl<1&&S.towers[2].lvl<1&&introT<30) waveT=Math.max(waveT,4); waveT-=dt; if(waveT<=0) startWave(); }
...
function updatePlayer(dt){
  ...
  else if(S.stones>=capS){ target=DEPOT; text='Taşı depola'; }
