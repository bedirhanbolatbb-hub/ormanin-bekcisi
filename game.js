...
function makeEnemy(kind,side){ const K=EK[kind]||EK.grunt; const w=gw(); const hp=Math.round(nightHp()*K.hp*((plan&&plan.mul&&plan.mul[side])||1)); let g, guy=null, pushers=null, ramLog=null, wolf=null; const sc=K.sc;
  ...
  enemies.push({g,guy,pushers,ramLog,wolf,kind,K,hp,maxHp:hp,boss:kind==='boss',sc,rad:K.rad,side,speed:3.4*K.spd*rand(0.97,1.03),atk:K.atk*atkK(),atkCd:0,atkT:kind==='ram'?1.6:1.0,dead:false,hitT:0,wp:1,off,bar,swing:0,ramT:0});
}
...
function pickKind(i,total){ const L=S.level, n=S.wave; if(n===WAVES&&i===total-1) return 'boss'; const r=Math.random(); if(L>=10){ const ks=['wolf','raider','knight','pirate','ice','poison','shield']; if(r<0.7) return ks[Math.floor(Math.random()*ks.length)]; } if(L>=9&&r<0.25) return 'ice'; if(L>=8&&r<0.25) return 'pirate'; if(L>=7&&r<0.22) return 'knight'; if(L>=6&&r<0.22) return 'poison'; if(L>=3&&r<0.26) return 'wolf'; if(L>=2&&r<0.4&&r>=0.2) return 'raider'; if(L>=3&&n>=3&&i%10===5) return 'ram'; if((L>=2||n>=4)&&r<0.16+0.02*L) return 'shield'; if(n>=2&&r<0.42+0.02*L) return 'runner'; return 'grunt'; }
...
function damageEnemy(e,dmg,src){ if(e.dead) return; let m=src&&e.K&&e.K[src]!==undefined? e.K[src] : 1; if(src==='arrow'&&e.kind==='knight'&&S.lv.ironArrow) m=Math.min(1,m+0.13*S.lv.ironArrow); e.hp-=dmg*m; if(e.boss&&!e.ph2&&e.hp>0) bossPhase(e); e.hitT=0.18; SFX.hit(); if(m<0.7&&src==='arrow'&&Math.random()<0.3) burst(e.g.position.clone().setY(1.2),3,M.metal,0.5); if(e.hp<=0) killEnemy(e); }
...
function renderHud(){ for(const L of quarryLabels) L.hide=!revealed('quarry');
  ...
  logsEl.textContent=S.logs; capEl.textContent='/'+D.cap(); $('woodStock').textContent=S.wood; $('stoneStock').textContent=S.stone+(S.stones?'+'+S.stones:''); $('stoneChip').style.display=revealed('quarry')?'flex':'none'; $('plankChip').style.display=revealed('river')?'flex':'none'; $('plankStock').textContent=S.planks||0; $('ironChip').style.display=revealed('iron')?'flex':'none'; $('ironStock').textContent=S.iron||0; $('potionChip').style.display=revealed('swamp')?'flex':'none'; $('potionStock').textContent=(S.rg&&S.rg.potions)||0; $('lootCount').textContent=S.loot;
