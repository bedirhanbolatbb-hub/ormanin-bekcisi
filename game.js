...
function completePad(pd){ const d=pd.def; S.paid[d.id]=0; pd.pop=1; pd.cool=1.2;
  ...
  SFX.build(); burst(pd.g.position.clone().setY(0.8),16,M.gold,1.2); if(d.kind!=='newTower') floatText(pd.g.position,d.name+(d.kind==='tower'||d.kind==='wall'?' ↑':' ↑'),'green'); save(); }
let bubblePad=null;
...
function updateEnemies(dt){
  ...
  else if(enemies.every(e=>e.dead)){ waveActive=false; if(S.wave>=WAVES){ levelComplete(); return; } S.wave++; waveT=Math.max(5,10-gw()*0.3); plan=null; planWave(); const bonus=15+gw()*5; toast(`Dalga temizlendi! +${bonus}`,'good'); const n=Math.min(40,10+gw()*2); dropCoins(new THREE.Vector3(0,2,4),n,bonus/n,4,1.2); save(); }
