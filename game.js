...
const PADS=[
  ...
  {id:'newCannon', ord:7, lock:'Gece 2', name:'Topçu Kulesi', desc:'Gülle atar; yerini sen seç', res:'gold', pos:P(5.4,-2.9), kind:'newTower', tk:'c', cost:l=>Math.round(90*Math.pow(1.6,l)), max:3, show:()=>S.level>1||S.wave>=2, lvl:()=>nFree('c')},
...
function cancelPlacing(){ if(!placing) return; scene.remove(placing.ghost.g); S.coins+=placing.refund; S.pendingRefund=0; if(placing.pd) placing.pd.needLeave=true; placing=null; $('placeBar').style.display='none'; toast('↩ Geri verildi','good'); }
...
function confirmPlace(){ if(!placing) return; const sp=placing.spot; if(!sp||!sp.ok){ toast('Buraya olmaz'); return; } scene.remove(placing.ghost.g); S.towers.push({k:placing.k,side:sp.s,a:0,x:sp.x,z:sp.z,px:sp.px,pz:sp.pz,lvl:1,fixed:false}); const i=S.towers.length-1; buildTower(i); ensureTowerPads(); applyCaps(); layoutPads(); S.pendingRefund=0; questEvent('buy',1); if(placing.pd) placing.pd.needLeave=true; placing=null; $('placeBar').style.display='none'; celebrate(new THREE.Vector3(sp.x,0,sp.z),1); floatText(new THREE.Vector3(sp.x,0,sp.z),'Kule kuruldu!','green'); save(); }
...
function regionGuide(mode){ const p=player.g.position; if(mode==='carry'){ const r6=regionGuide6('carry'); if(r6) return r6; } if(mode==='idle'){ const r6=regionGuide6('idle'); if(r6) return r6; }
  if(mode==='carry'){ const pileT=P=>pileVal(P)>=1?{t:P.pos,text:'Altınları topla'}:null;
    if((S.meat||0)>0&&revealed('meadow')){ if((S.rg.huntMeat||0)<hhutCap()) return {t:HHUT_FRONT,text:'Eti av kulübesine götür'}; const g=pileT(meatPile); if(g) return g; }
    const whOk=revealed('coast')&&pileVal(coastPile)<coastPile.capFn()-0.5, hutOk=revealed('lake')&&(S.rg.hutFish||0)<hutCap();
    if((S.fish||0)>0&&whOk&&(!hutOk||p.distanceTo(WH_FRONT)<p.distanceTo(HUT_FRONT))) return {t:WH_FRONT,text:'Balıkları limana sat'};
    if((S.fish||0)>0&&hutOk) return {t:HUT_FRONT,text:'Balıkları balıkhaneye götür'};
...
const P6=[
  ...
  {id:'cauldron', grp:'swamp', ord:4, name:'İksir Kazanı', desc:'İksir gece suru onarır', res:'gold', pos:padAt(SW,1.4), kind:'up', key:'cauldron', cost:l=>Math.round(200*Math.pow(1.6,l)), max:5, show:()=>revealed('swamp'), onBuy:()=>{ celebrate(CAUL.clone(),1.2); if(SWP.post) powerOn(SWP.post); }},
  ...
  {id:'drill', grp:'iron', ord:4, lock:'Bir madenci', name:'Maden Matkabı', desc:'Makine: cevher kendiliğinden çıkar', res:'gold', pos:padAt(IR,0.2), kind:'up', key:'drill', cost:l=>Math.round(620*Math.pow(1.75,l)), max:5, show:()=>revealed('iron')&&rg('miner')>=1, onBuy:()=>{ celebrate(DRILL.clone(),1.6); camShake=0.5; if(DRL.post) powerOn(DRL.post); DRL.ph=0.9; }},
  {id:'forge', grp:'iron', ord:5, name:'Demirci Ocağı', desc:'Hızlı eritir, çok tutar', res:'gold', pos:padAt(IR,3.0), kind:'up', key:'forge', cost:l=>Math.round(260*Math.pow(1.6,l)), max:5, show:()=>revealed('iron'), onBuy:()=>{ celebrate(FORGE.clone(),1.2); if(FRG.post) powerOn(FRG.post); }},
  ...
  {id:'boat', grp:'coast', ord:2, name:'Ticaret Teknesi', desc:'Makine: adalardan para getirir', res:'gold', pos:padAt(CO,-3.4), kind:'up', key:'boat', cost:l=>Math.round(760*Math.pow(1.7,l)), max:5, show:()=>revealed('coast'), onBuy:()=>{ celebrate(PIER_B.clone(),1.4); camShake=0.4; if(CST.post) powerOn(CST.post); }},
  ...
  {id:'jeweler', grp:'snow', ord:1, name:'Kuyumcu', desc:'Kristali pahalı ve hızlı satar', res:'gold', pos:padAt(SN,-6.2), kind:'up', key:'jeweler', cost:l=>Math.round(460*Math.pow(1.6,l)), max:5, show:()=>revealed('snow'), onBuy:()=>{ celebrate(JEW.clone(),1.2); if(SNW.post) powerOn(SNW.post); }},
  ...
  {id:'cdrill', grp:'snow', ord:3, lock:'Bir kristalci', name:'Kristal Matkabı', desc:'Makine: kristal kendiliğinden çıkar', res:'gold', pos:padAt(SN,-0.6), kind:'up', key:'cdrill', cost:l=>Math.round(1100*Math.pow(1.75,l)), max:5, show:()=>revealed('snow')&&rg('cminer')>=1, onBuy:()=>{ celebrate(CDRILL.clone(),1.8); camShake=0.6; if(SNW.dpost) powerOn(SNW.dpost); }},
];
...
function regionGuide6(mode){ const p=player.g.position; const near=(list)=>{ let best=null,bd=1e9; for(const n of list){ if(!n.alive||n.claimed) continue; const d=Math.hypot(n.pos.x-p.x,n.pos.z-p.z); if(d<bd){ bd=d; best=n; } } return best; };
  if(mode==='carry'){ if((S.ore||0)>0&&revealed('iron')&&(S.rg.oreIn||0)<oreCap()) return {t:FORGE_FRONT,text:'Cevheri demirciye götür'}; if((S.crystal||0)>0&&revealed('snow')&&(S.rg.crysIn||0)<crysCap()) return {t:JEW_FRONT,text:'Kristali kuyumcuya götür'}; if((S.herb||0)>0&&revealed('swamp')&&(S.rg.herbIn||0)<herbCap()) return {t:SHUT_FRONT,text:'Mantarı otacıya götür'}; return null; }
