...
const BAL=new THREE.Vector3(-9,0,-76), BAL_RES=['stone','gold','plank','wood','gold'], BAL_COST=[2000,8000,600,3500,20000];
...
function updateBallista(dt){ const dT=(S.level===10&&!(S.book&&S.book.boss&&S.book.boss[10]))?1:0; DOOM+=(dT-DOOM)*Math.min(1,dt*(DOOM>1?0.6:0.8)); /* F9b: son sefer ışığı */
  ...
  for(let i=bolts.length-1;i>=0;i--){ const b=bolts[i], t=b.target; if(t.dead){ scene.remove(b.m); bolts.splice(i,1); continue; } const to=t.g.position.clone(); to.y=1.2; const d=to.clone().sub(b.m.position), dl=d.length(); if(dl<1.0){ scene.remove(b.m); bolts.splice(i,1); damageEnemy(t,b.dmg,'bolt'); burst(to,10,M.woodDark,1.1,1.3); SFX.hit(); continue; } b.m.position.addScaledVector(d.normalize(),Math.min(dl,40*dt)); b.m.lookAt(to); }
...
function bossPhase(e){ if(!e.boss||e.ph2||e.dead||bossOf().hat!=='crown'||e.hp>e.maxHp*0.65) return; e.ph2=true; e.chanT=3; e.cdn=4;
  ...
  banner(T('⚠ KARA KRAL ÖFKELENİYOR','⚠ THE BLACK KING GATHERS RAGE'),T('3 sn · Kılıcınla ona vurmaya hazırlan','3 s · get ready to strike him with your sword'),'boss'); tone(90,180,2.8,'sawtooth',0.05); camShake=Math.max(camShake,0.25); }
...
function kingEnrage(e){ curseSaid=false; if(e.enrRing){ e.g.remove(e.enrRing); e.enrRing.geometry.dispose(); e.enrRing.material.dispose(); e.enrRing=null; } e.speed*=1.2; e.atk*=1.3; e.ward=WARD_N; const pos=e.g.position.clone();
  banner(T('KARA KRAL ÖFKELENDİ!','BLACK KING ENRAGED!'),T('Kalkan iksiri keser · Kılıçla kır!','Ward blocks potions · Break it with your sword!'),'boss'); SFX.night(); SFX.boom(); camShake=0.7; burst(pos.clone().setY(1.5),24,M.darkRoof,1.6,1.6); const aura=glow(0xb040ff,6,0.6); aura.position.y=2; e.g.add(aura); e.aura=aura;
...
function endlessIntro(card){ const n1=eNight(LEVELS+1,1);
  card.innerHTML=`<div class="card result endless"><h1>♾ ${ENAME()}</h1><p class="boss">${T('Orman güvende, ama Kara Kral\'ın orduları gelmeye devam ediyor.','The forest is safe, but the Black King\'s armies keep coming.')}</p><ul class="egoal"><li><i>🌙</i><span>${T('<b>Olabildiğince çok gece dayan.</b> Her gece biraz daha güçlüler.','<b>Survive as many nights as you can.</b> Each night is a little stronger.')}</span></li><li><i>💀</i><span>${T('Her 5. gece bir patron gelir.','A boss comes every 5th night.')}</span></li><li><i>🏅</i><span>${T('Her 5 gecede: 👑 taç + 🃏 güç kartı.','Every 5 nights: 👑 crowns + a 🃏 power card.')}</span></li><li><i>⬆</i><span>${T('Kuleler, sur ve askerler her patrondan sonra daha da yükselir.','Towers, Walls and Soldiers level higher after every boss.')}</span></li><li><i>🏆</i><span>${T('Kaybedersen aynı gece baştan başlar. Rekorunu geç!','Lose and the same night starts over. Beat your record!')}</span></li></ul><button id="winNext">${T(`Başla · Gece ${n1} →`,`Begin · Night ${n1} →`)}</button></div>`;
